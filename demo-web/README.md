...
Текущий доступ с телефона в локальной сети: `https://192.168.1.74:8443/` (самоподписанный
сертификат надо принять на устройстве). С Windows-хоста тот же backend сейчас доступен по
`https://172.16.15.72:8443/`: прямое обращение Windows к собственному LAN-IP `192.168.1.74:8443`
в этом WSL mirrored окружении тайм-аутится. Это два адреса одного web-сервиса, а не два сервера.
Открытие страницы по HTTP/IP не даёт браузеру доступа к микрофону; для звонка используйте HTTPS.

`frontend/demo-config.js` содержит QR URL `https://192.168.1.74:8443/`, SIP-учётную запись `1000`
и очередь `sip:7100@192.168.1.74`. FreeSWITCH по-прежнему запущен в отдельном WSL-дистрибутиве
...
Caller ID страницы передаётся как SIP URI user-part. Перед публичной демонстрацией замените IP,
QR-код, сертификат и demo credentials.
