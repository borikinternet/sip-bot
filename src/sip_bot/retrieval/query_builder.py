...
class KnowledgeQueryBuilder:
    ...
    def build(self, text: str, *, context: Iterable[tuple[str, str]] = ()) -> KnowledgeQuery:
        ...
        terms = tuple(dict.fromkeys(token for token in normalized_tokens if token not in _STOP_WORDS or token in _NEGATIONS))
