...
и внутренним входом модели 16 kHz, XTTS и первый TTS PCM chunk. Warmup занял `21350.216 ms`; от admission вызова до
финального отчёта — `101264.674 ms` в полном сценарии, включая управляемые паузы fixture.

XTTS вывел предупреждение о длине отдельных русских ответов относительно лимита tokenizer `182` символа; это не
нарушило acceptance, но остаётся кандидатом для последующего качества/латентности в карте 5. Высокое число
`egress_underruns` в PJMEDIA — наблюдаемое ограничение текущего демонстрационного paced-output стенда, не ошибка
callback или потеря RTP; отдельные production hardening/quality исправления не подменяются этим closeout.
